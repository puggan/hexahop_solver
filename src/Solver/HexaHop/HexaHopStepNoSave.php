...
(static function (int $level_number) {
    ...
    $solver = new SolverNoSave($start_state, new IniHashStorage($solved_filename));
    ...
    if (is_file($solved_filename)) {
        ...
        if (is_file($stat_filename)) {
            ...
            if (!$stats[$level_number]->steps || $stats[$level_number]->steps > $steps) {
                ...
                file_put_contents($stat_filename, json_encode($stats, JSON_PRETTY_PRINT | JSON_THROW_ON_ERROR));
            }
        }
